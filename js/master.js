...
site.init = function() {
  ...
  /**
   * site.define() - Define a site.js module
   * @param  {string} name
  ...
  /**
   * site.require() - Load a site.js module
   * @param  {string} name
   * @return {object}
   */
  ...
  // site.destroy() - Trigger a cleanup event for all modules
...
site.define('ix', function($, _) {
  ...
  function init() {
    ...
    if (anchors.length) {
      site.scroll.on(scroll);
      setTimeout(scroll, 1);
    }

    // Handle loads or readys if they exist
    if (loads.length) site.load(runLoads);
  ...
  // Events used by other site modules
  var events = {
    reset: function(i, el) {
      el.__wf_intro = null;
    },
...
site.define('forms', function($, _) {
  ...
  function build(i, el) {
    ...
    // site form
    if (siteId) { data.handler = submitsite; return; }

    // Alert for disconnected site forms
    disconnected();
  }
  ...
  function addListeners() {
    ...
    // Handle form submission for site forms
  ...
  function submitsite(data) {
    ...
    var url = 'https://site.com/api/v1/form/' + siteId;

    // Work around same-protocol IE XDR limitation - without this IE9 and below forms won't submit
    if (retro && url.indexOf('https://site.com') >= 0) {
      url = url.replace('https://site.com/', 'http://formdata.site.com/');
    }
  ...
  var disconnected = _.debounce(function() {
    alert('Oops! This page has a form that is powered by site, but important code was removed that is required to make the form work. Please contact support@site.com to fix this issue.');
  }, 100);
...
site.define('maps', function($, _) {
  ...
  api.preview = function() {
    ...
    site.resize.off(triggerRedraw);
    if ($maps.length) {
      site.resize.on(triggerRedraw);
      triggerRedraw();
    }
  };
  ...
  function triggerRedraw() {
    if ($maps.length && site.app) {
      $maps.each(site.app.redrawElement);
    }
  }
  ...
  function removeListeners() {
    site.resize.off(resizeMaps);
    site.redraw.off(resizeMaps);
  }

  function addListeners() {
    site.resize.on(resizeMaps);
    site.redraw.on(resizeMaps);
  }
  ...
  function getState(el, data) {
    ...
    var mapDraggable = (site.env.touch && data.disableTouch) ? false : true;
...
/**
 * ----------------------------------------------------------------------
 * site: Auto-select links to current page or section
 */
site.define('links', function($, _) {
  'use strict';
  ...
  function init() {
    ...
    slug = site.env('slug') || location.pathname || '';
...
site.define('lightbox', function ($, _) {
  'use strict';
...
site.define('navbar', function($, _) {
  ...
  function removeListeners() {
    site.resize.off(resizeAll);
  }

  function addListeners() {
    site.resize.on(resizeAll);
  }
 ...
 */
site.define('dropdown', function($, _) {
  'use strict';
